...
from phase2.nn_models import CNN1DRegressor
from phase2.nn_train import make_loaders, predict, train_regressor


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase2 baseline: 1D CNN")
...
def main() -> None:
    ...
    model = CNN1DRegressor(input_dim=data.x.shape[1]).to(device)
    ...
    metrics = save_method_outputs(cfg, "cnn1d", y_true, y_pred)

    out_dir = method_dir(cfg, "cnn1d")
    torch.save(model.state_dict(), out_dir / "model.pt")
    print(f"Saved model to: {out_dir / 'model.pt'}")
    print(f"1D CNN | RMSE={metrics['rmse']:.6f} nm | MAE={metrics['mae']:.6f} nm | R2={metrics['r2']:.6f}")
